...
from scapy.fields import ByteEnumField, ByteField, Field, FieldLenField, \
    FieldListField, FlagsField, BitEnumField, ShortField, XIntField, IntField, LEShortEnumField, LEShortField, \
...
class BB_RF(Packet):
    ...
    fields_desc = [
        ...
        ByteField("singnal_power", 0),
        ByteField("noise_power", 0),
        ByteField("access_code_offense", 0),
        ByteField("payload_transport_rate", 0x30),
        ByteField("corrected_header_bits", 0),
        ShortField("corrected_payload_bits", 0),
        StrFixedLenField("lower_address_part", "\x3a\xa5\x60\x00",4),
        StrFixedLenField("reference_lap", "\x3a\xa5\x60",3),
        ByteField("reference_uap", 0x70),
        LEIntField("bt_packet_header", 0),
        LEShortEnumField("flags", 0x00b7,{
            0x0001: "de-whitened",
            0x0002: "Signal_Valid",
            0x0004: "Noise_Valid",
            0x0008: "Decrypted",
            0x0010: "LAP_valid",
            0x0020: "Payload_present",
            0x0040: "RF_channel_aliasing",
            0x0080: "UAP_valid",
            0x0100: "Hec_checked",
            0x0200: "hec_passed",
            0x0400: "crc_checked",
...
class ESP32_BREDR(Packet):
    ...
    fields_desc = [
        ...
        BitField("rx_enc", 0, 1),
        BitField("tx_enc", 0, 1),
        BitField("intercept_req", 0, 1),
        BitField("retry_flag", 0, 1),
        BitField("custom_lmp", 0, 1),
...
class BT_ACL_Hdr(Packet):
    ...
    fields_desc = [
        ...
        BitEnumField("llid", 0, 2, {0x00: 'undefined',
                                    ...
                                    0x03: 'LMP'}),
        BitField("rfu", 0, 3),
        BitFieldLenField("len_2", 0, 5),
        
    ]
    def post_build(self, p, pay):
        p += pay
        if self.len_1 == 0 and self.len_2 == 0:
            l_pay = len(pay)
            
            self.len_1 = l_pay >> 5 & 0b11111
            self.len_2 = l_pay & 0b11111
            p = chb(self.len_2 << 3 | self.flow<<2 | self.llid) + chb(self.rfu << 5 | self.len_1) + p[2:]
        return p
...
class LMP_features_req(Packet):
    ...
    fields_desc = [FlagsField(
        "features", 0x9feecdfedbff7b87, 64, _bluetooth_lmp_features)]
...
class LMP_channel_classification_req(Packet):
    ...
    fields_desc = [
        ...
        LEShortField("min_interval", 0x0c80),
        LEShortField("max_interval", 0x1900),
    ]
...
class FHS(Packet):
    name = "FHS"
    fields_desc = [
        BitField("Parity_bits",0,34),
        BitField("LAP",0,24),
        BitField("EIR",0,1),
        BitField("Reserverd",0,1),
        BitField("SR",0,2),
        BitField("SP",0,2),
        BitField("UAP",0,8),
        BitField("NAP",0,16),
        BitField("ClassodDevice",0,24),
        BitField("LT_ADDR",0,3),
        BitField("CLK",0,26),
        BitField("Page_scan_mode",0,3),
    ]

    
conf.l2types.register(255,BB_RF)
bind_layers(BB_RF, BT_Baseband)
bind_layers(ESP32_BREDR, BT_Baseband)
bind_layers(BT_Baseband, FHS, type=0x02)
